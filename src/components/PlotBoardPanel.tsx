...
export function PlotBoardPanel({ board, entities, draftSeed, onDraftSeedApplied, onSave }: PlotBoardPanelProps) {
  ...
  useEffect(() => {
    ...
    setEditor({ id: undefined, laneId: lanes[0].id, spanToLaneId: '', phaseId: phases[0].id, title: draftSeed.title ?? '', notes: '', status: 'seed', entityId: draftSeed.entityIds?.find((id) => entityById.has(id)) ?? '', linkTargetId: '', linkLabel: 'leads to' });
    onDraftSeedApplied?.();
  ...
  const openNewBeat = (laneId: string, phaseId: string) => setEditor({ laneId, spanToLaneId: '', phaseId, title: '', notes: '', status: 'seed', entityId: '', linkTargetId: '', linkLabel: 'leads to' });
  const openBeat = (beat: PlotBoardBeat) => {
    const outgoing = draft?.links.find((link) => link.sourceBeatId === beat.id);
    const spanEnd = [...(beat.spanLaneIds ?? [])].filter((laneId) => laneId !== beat.laneId).at(-1) ?? '';
    setEditor({ id: beat.id, laneId: beat.laneId, spanToLaneId: spanEnd, phaseId: beat.phaseId, title: beat.title, notes: beat.notes, status: beat.status, entityId: beat.entityIds[0] ?? '', linkTargetId: outgoing?.targetBeatId ?? '', linkLabel: outgoing?.label ?? 'leads to' });
  };
  const saveBeat = () => {
    ...
    const anchorIndex = lanes.findIndex((lane) => lane.id === editor.laneId);
    const endIndex = lanes.findIndex((lane) => lane.id === editor.spanToLaneId);
    const spanLaneIds = endIndex > anchorIndex ? lanes.slice(anchorIndex, endIndex + 1).map((lane) => lane.id) : [];
    const beat: PlotBoardBeat = existing
      ? { ...existing, laneId: editor.laneId, ...(spanLaneIds.length > 1 ? { spanLaneIds } : { spanLaneIds: undefined }), phaseId: editor.phaseId, title: editor.title.trim(), notes: editor.notes.trim(), status: editor.status, entityIds, updatedAt: timestamp }
      : { id: crypto.randomUUID(), laneId: editor.laneId, ...(spanLaneIds.length > 1 ? { spanLaneIds } : {}), phaseId: editor.phaseId, title: editor.title.trim(), notes: editor.notes.trim(), status: editor.status, entityIds, order: draft.beats.filter((item) => item.laneId === editor.laneId && item.phaseId === editor.phaseId).length, createdAt: timestamp, updatedAt: timestamp };
  ...
  const removeLane = (laneId: string) => {
    if (!draft || draft.beats.some((beat) => beat.laneId === laneId || beat.spanLaneIds?.includes(laneId))) return;
    const timestamp = new Date().toISOString();
  ...
  const isSpanning = (beat: PlotBoardBeat) => (beat.spanLaneIds?.length ?? 0) > 1;
  const spanEndIndex = (beat: PlotBoardBeat) => Math.max(...(beat.spanLaneIds ?? [beat.laneId]).map((laneId) => lanes.findIndex((lane) => lane.id === laneId)));
    ...
    {!draft ? <div className="plot-board-empty"><h3>Start a plot board</h3><p>Build the campaign outline manually. Existing timeline entries are not included.</p><button className="primary-button" onClick={createBoard} type="button">Start plot board</button></div> : <>
      ...
      {!phases.length || !lanes.length ? <div className="plot-board-guidance"><strong>{!phases.length ? 'Add your first phase.' : 'Add your first story arc.'}</strong><span>Phases are vertical campaign sections; arcs are horizontal narrative tracks.</span></div> : <>
        ...
        <div className="plot-board-scroll"><div className="plot-board-grid" style={{ '--phase-count': phases.length } as CSSProperties}>
          <div className="plot-board-corner">Story arcs</div>{phases.map((phase, phaseIndex) => <div className="plot-board-phase" key={phase.id}><strong>{phase.title}</strong><div className="plot-board-item-actions"><button aria-label={`Move ${phase.title} left`} disabled={phaseIndex === 0} onClick={() => movePhase(phase.id, -1)} type="button">←</button><button aria-label={`Insert a phase after ${phase.title}`} onClick={() => preparePhaseInsert(phase.id)} type="button">+</button><button aria-label={`Move ${phase.title} right`} disabled={phaseIndex === phases.length - 1} onClick={() => movePhase(phase.id, 1)} type="button">→</button><button aria-label={`Remove ${phase.title}`} disabled={draft.beats.some((beat) => beat.phaseId === phase.id)} onClick={() => removePhase(phase.id)} type="button">×</button></div></div>)}
          {lanes.flatMap((lane, laneIndex) => [
            <div className={`plot-board-lane tone-${lane.tone}`} key={`${lane.id}:label`}><strong>{lane.title}</strong><span>{lane.tone}</span><div className="plot-board-item-actions plot-board-lane-actions"><button aria-label={`Move ${lane.title} up`} disabled={laneIndex === 0} onClick={() => moveLane(lane.id, -1)} type="button">↑</button><button aria-label={`Insert a story arc after ${lane.title}`} onClick={() => prepareLaneInsert(lane.id)} type="button">+</button><button aria-label={`Move ${lane.title} down`} disabled={laneIndex === lanes.length - 1} onClick={() => moveLane(lane.id, 1)} type="button">↓</button><button aria-label={`Remove ${lane.title}`} disabled={draft.beats.some((beat) => beat.laneId === lane.id || beat.spanLaneIds?.includes(lane.id))} onClick={() => removeLane(lane.id)} type="button">×</button></div></div>,
            ...phases.map((phase) => <div className="plot-board-cell" key={`${lane.id}:${phase.id}`}><div className="plot-board-beats">{beats.filter((beat) => beat.laneId === lane.id && beat.phaseId === phase.id && !isSpanning(beat)).map((beat) => renderBeat(beat))}</div><button aria-label={`Add a beat to ${lane.title} in ${phase.title}`} className="plot-board-add-beat" onClick={() => openNewBeat(lane.id, phase.id)} type="button">+ Beat</button></div>)
          ])}
          {beats.filter(isSpanning).map((beat) => {
            ...
            return <div className="plot-board-span" key={`span:${beat.id}`} style={{ gridColumn: phaseIndex + 2, gridRow: `${laneIndex + 2} / ${endIndex + 3}` }}>{renderBeat(beat, 'plot-board-beat plot-board-span-beat')}</div>;
          })}
        </div></div>
      </>}
      {editor && <section className="plot-beat-editor" aria-label="Plot beat editor"><div><h3>{editor.id ? 'Edit plot beat' : 'New plot beat'}</h3><button onClick={() => setEditor(null)} type="button">Close</button></div><label>Story arc<select onChange={(event) => setEditor({ ...editor, laneId: event.target.value, spanToLaneId: '' })} value={editor.laneId}>{lanes.map((lane) => <option key={lane.id} value={lane.id}>{lane.title}</option>)}</select></label><label>Span to arc<select onChange={(event) => setEditor({ ...editor, spanToLaneId: event.target.value })} value={editor.spanToLaneId}><option value="">This arc only</option>{lanes.slice(Math.max(0, lanes.findIndex((lane) => lane.id === editor.laneId) + 1)).map((lane) => <option key={lane.id} value={lane.id}>Through {lane.title}</option>)}</select></label><label>Phase<select onChange={(event) => setEditor({ ...editor, phaseId: event.target.value })} value={editor.phaseId}>{phases.map((phase) => <option key={phase.id} value={phase.id}>{phase.title}</option>)}</select></label><label>Status<select onChange={(event) => setEditor({ ...editor, status: event.target.value as PlotBeatStatus })} value={editor.status}>{(Object.keys(statusLabel) as PlotBeatStatus[]).map((status) => <option key={status} value={status}>{statusLabel[status]}</option>)}</select></label><label>Title<input autoFocus onChange={(event) => setEditor({ ...editor, title: event.target.value })} placeholder="The discovery, betrayal, revelation…" value={editor.title} /></label><label>Reference<select onChange={(event) => setEditor({ ...editor, entityId: event.target.value })} value={editor.entityId}><option value="">No Worldbuilding reference</option>{entities.map((entity) => <option key={entity.id} value={entity.id}>{entity.name}</option>)}</select></label><label>Connect to<select onChange={(event) => setEditor({ ...editor, linkTargetId: event.target.value })} value={editor.linkTargetId}><option value="">No direct connection</option>{beats.filter((beat) => beat.id !== editor.id).map((beat) => <option key={beat.id} value={beat.id}>{beat.title}</option>)}</select></label><label>Connection label<input disabled={!editor.linkTargetId} onChange={(event) => setEditor({ ...editor, linkLabel: event.target.value })} value={editor.linkLabel} /></label><label className="plot-beat-notes">Notes<textarea onChange={(event) => setEditor({ ...editor, notes: event.target.value })} placeholder="What happens, why it matters, what can change…" value={editor.notes} /></label><div className="plot-beat-editor-actions"><button className="primary-button" onClick={saveBeat} type="button">Save beat</button>{editor.id && <button className="quiet-danger" onClick={removeBeat} type="button">Remove beat</button>}</div></section>}
    </>}
  </section>;
}
