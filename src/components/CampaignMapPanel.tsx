...
type DisplayNode = { id: string; label: string; subtitle: string; notes: string; width: number; height: number; x: number; y: number; tone: string; entityId?: string; brewId?: string; encounterId?: string };
...
function MapCanvas({ nodes, links, selectedId, editable = false, onMove, onMoveEnd, onConnect, onNodeClick }: MapCanvasProps) {
  ...
  const move = (event: PointerEvent<HTMLDivElement>) => {
    ...
    if (!position) return;
    if (draggingId && onMove) onMove(draggingId, clamp(position.x, 4, 96), clamp(position.y, 7, 93));
    if (linkingSourceId) setLinkPointer(position);
  };
  ...
  return <div className={`campaign-map-canvas ${editable ? 'is-editable' : ''} ${linkingSourceId ? 'is-linking' : ''}`} onPointerCancel={cancelInteraction} onPointerLeave={(event) => { if (draggingId) finishInteraction(event); else if (linkingSourceId) cancelInteraction(); }} onPointerMove={move} onPointerUp={finishInteraction} ref={surfaceRef}>
    ...
    })}{linkingSource && linkPointer && <line className="campaign-map-link-preview" x1={linkingSource.x} x2={linkPointer.x} y1={linkingSource.y} y2={linkPointer.y} />}</svg>
    {nodes.map((node) => <div className={`campaign-map-node tone-${node.tone} ${selectedId === node.id ? 'is-selected' : ''} ${linkingSourceId === node.id ? 'is-link-source' : ''}`} data-map-node-id={node.id} key={node.id} style={{ height: `${node.height}px`, left: `${node.x}%`, top: `${node.y}%`, width: `${node.width}px` }}>
      <button aria-pressed={selectedId === node.id} className="campaign-map-node-select" onClick={() => onNodeClick(node)} type="button"><strong>{node.label}</strong><span>{node.subtitle}</span>{node.notes && <small>{node.notes}</small>}</button>
      {editable && <div className="campaign-map-node-actions"><button aria-label={`Move ${node.label}`} className="campaign-map-drag-handle" onPointerDown={(event) => { event.preventDefault(); event.stopPropagation(); event.currentTarget.setPointerCapture(event.pointerId); setDraggingId(node.id); }} type="button">⠿</button><button aria-label={`Connect ${node.label} to another node`} className="campaign-map-connect-handle" onPointerDown={(event) => { event.preventDefault(); event.stopPropagation(); event.currentTarget.setPointerCapture(event.pointerId); setLinkingSourceId(node.id); setLinkPointer({ x: node.x, y: node.y }); }} type="button">●</button></div>}
    </div>)}
  </div>;
}
...
export function CampaignMapPanel({ brews, campaignMap, currentStateByEntityId, entities, entityReferences, worldbuildingEntries, onSave }: CampaignMapPanelProps) {
  ...
  const addNote = () => {
    ...
    replaceDraft({ ...draft, nodes: [...draft.nodes, node], updatedAt: timestamp }); setNote(''); selectNode(node.id);
  };
  const addEntity = (id = entityId) => {
    ...
    replaceDraft({ ...draft, nodes: [...draft.nodes, node], updatedAt: timestamp }); setEntityId(''); setReferencedEntityId(''); selectNode(node.id);
  };
  const connectNodes = (fromId: string, toId: string) => {
    ...
    const timestamp = new Date().toISOString();
    const link: CampaignMapLink = { id: crypto.randomUUID(), sourceId: fromId, targetId: toId, label: linkLabel.trim() || 'related to', createdAt: timestamp };
    replaceDraft({ ...current, links: [...current.links, link], updatedAt: timestamp });
  };
  const addLink = () => {
    if (!sourceId || !targetId) return;
    connectNodes(sourceId, targetId); setSourceId(''); setTargetId(''); setLinkLabel('related to');
  };
  const saveSelectedNode = () => {
    const current = draftRef.current;
    if (!current || !selectedId || !selectedLabel.trim()) return;
    const timestamp = new Date().toISOString();
    const meta: NodeMeta = { notes: selectedNotes.trim(), width: clamp(selectedWidth, 100, 280), height: clamp(selectedHeight, 48, 200) };
    replaceDraft({
      ...current,
  ...
  const loadReferenceCluster = () => {
    ...
    targets.forEach((entity, index) => {
      ...
      const target = ensureEntity(entity, Math.round(clamp(source.x + Math.cos(angle) * 28, 8, 92)), Math.round(clamp(source.y + Math.sin(angle) * 28, 10, 90)));
  ...
  const nudgeSelected = (x: number, y: number) => {
    ...
    if (!node) return;
    const timestamp = new Date().toISOString();
    replaceDraft({ ...draft, nodes: draft.nodes.map((item) => item.id === node.id ? { ...item, x: clamp(item.x + x, 4, 96), y: clamp(item.y + y, 7, 93), updatedAt: timestamp } : item), updatedAt: timestamp });
  };
