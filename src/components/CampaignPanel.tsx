...
export function CampaignPanel({ position, partyLocation, brews, encounters, entities, currentStateByEntityId, worldEvents, timelineEntries, onOpenEncounter, onOpenEntity, onOpenBrewSection, onSaveTimelineEntry, onDeleteTimelineEntry }: CampaignPanelProps) {
  ...
  const [draft, setDraft] = useState({ lane: 'main' as TimelineLane, status: 'planned' as TimelineStatus, title: '', when: '', notes: '', entityIds: [] as string[], dateEra: 'AA' as BelentorEra, dateYear: '', dateMonth: 'Din' as BelentorMonth, dateDay: '', encounterId: '', brewId: '', sectionId: '' });
  ...
  const visualEvents = useMemo(() => [
    ...orderedTimelineEntries.map((entry) => ({ id: `timeline:${entry.id}`, lane: entry.lane, status: entry.status, title: entry.title, subtitle: timelineDateLabel(entry) || entry.status, system: false, brewId: entry.brewId, sectionId: entry.sectionId })),
    ...worldEvents.map((event) => ({ id: `event:${event.id}`, lane: 'main' as const, status: 'past' as const, title: event.type.replaceAll('.', ' '), subtitle: new Date(event.occurredAt).toLocaleString(), system: true, brewId: undefined, sectionId: undefined }))
  ...
  const submitTimeline = () => {
    ...
    onSaveTimelineEntry({ ...draft, when: date ? formatBelentorDate(date) : '', date, encounterId: draft.encounterId || undefined, brewId: draft.brewId || undefined, sectionId: draft.sectionId || undefined });
    setDraft({ lane: draft.lane, status: 'planned', title: '', when: '', notes: '', entityIds: [], dateEra: 'AA', dateYear: '', dateMonth: 'Din', dateDay: '', encounterId: '', brewId: '', sectionId: '' });
  };
