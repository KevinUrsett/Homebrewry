...
export function CampaignPanel({ position, partyLocation, brews, encounters, entities, currentStateByEntityId, worldEvents, timelineEntries, onOpenEncounter, onOpenEntity, onOpenBrewSection, onSaveTimelineEntry, onDeleteTimelineEntry }: CampaignPanelProps) {
  ...
  const [draft, setDraft] = useState({ lane: 'main' as TimelineLane, status: 'planned' as TimelineStatus, title: '', when: '', notes: '', entityIds: [] as string[], encounterId: '', brewId: '', sectionId: '' });
  ...
  const visualEvents = useMemo(() => [
    ...timelineEntries.map((entry) => ({ id: `timeline:${entry.id}`, lane: entry.lane, status: entry.status, title: entry.title, subtitle: entry.when || entry.status, system: false, brewId: entry.brewId, sectionId: entry.sectionId })),
  ...
  ], [timelineEntries, worldEvents]);
  const submitTimeline = () => {
    if (!draft.title.trim()) return;
    onSaveTimelineEntry({ ...draft, encounterId: draft.encounterId || undefined, brewId: draft.brewId || undefined, sectionId: draft.sectionId || undefined });
    setDraft({ lane: draft.lane, status: 'planned', title: '', when: '', notes: '', entityIds: [], encounterId: '', brewId: '', sectionId: '' });
  };
