...
function MapCanvas({ nodes, links, selectedId, editable = false, onMove, onMoveEnd, onNodeClick, onNodeDragStart }: { nodes: readonly DisplayNode[]; links: readonly DisplayLink[]; selectedId?: string | null; editable?: boolean; onMove?: (id: string, x: number, y: number) => void; onMoveEnd?: () => void; onNodeClick: (node: DisplayNode) => void; onNodeDragStart?: (node: DisplayNode) => void }) {
  ...
  return <div className={`campaign-map-canvas ${editable ? 'is-editable' : ''}`} onPointerMove={move} onPointerUp={finishMove} onPointerLeave={finishMove} ref={surfaceRef}>
    ...
    {nodes.map((node) => <div className={`campaign-map-node tone-${node.tone} ${selectedId === node.id ? 'is-selected' : ''}`} key={node.id} style={{ left: `${node.x}%`, top: `${node.y}%` }}>
      ...
      {editable && <button aria-label={`Move ${node.label}`} className="campaign-map-drag-handle" onPointerDown={(event) => { event.preventDefault(); event.currentTarget.setPointerCapture(event.pointerId); onNodeDragStart?.(node); setDraggingId(node.id); }} type="button">⠿</button>}
    </div>)}
  </div>;
}

export function CampaignMapPanel({ brews, campaignMap, currentStateByEntityId, entities, entityReferences, worldbuildingEntries, onOpenEntity, onOpenBrew, onSave }: CampaignMapPanelProps) {
  ...
  const boardNodes: DisplayNode[] = (draft?.nodes ?? []).map((node) => {
    ...
    return { id: node.id, label: node.label, subtitle: entity ? String(currentStateByEntityId.get(entity.id)?.fields.status?.value ?? entity.kind) : node.brewId ? 'Brew source' : 'Note', x: node.x, y: node.y, tone: entity ? 'entity' : 'note', entityId: node.entityId, brewId: node.brewId };
  });
  const addNote = () => {
  ...
  const loadReferenceCluster = () => {
    ...
    if (loadKind === 'brew') {
      ...
      if (!source.brewId) source.brewId = brew.id;
  ...
  const openMapNode = (node: DisplayNode) => {
    setSelectedId(node.id);
    const entity = node.entityId ? entities.find((item) => item.id === node.entityId) : undefined;
    if (entity) { onOpenEntity(entity); return; }
    if (node.brewId) { onOpenBrew(node.brewId); return; }
    // Older saved boards did not retain the brew ID, so recover it from the source label when possible.
    const legacyTitle = node.label.startsWith('Brew: ') ? node.label.slice(6) : '';
    const legacyBrew = legacyTitle ? brews.find((brew) => (brew.title || 'Untitled Brew') === legacyTitle) : undefined;
    if (legacyBrew) onOpenBrew(legacyBrew.id);
  };
