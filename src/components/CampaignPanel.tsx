...
export function CampaignPanel({ position, partyLocation, brews, encounters, entities, currentStateByEntityId, worldEvents, timelineEntries, onOpenEncounter, onOpenEntity, onSaveTimelineEntry, onDeleteTimelineEntry }: CampaignPanelProps) {
  ...
  const visualEvents = useMemo(() => [
    ...timelineEntries.map((entry) => ({ id: `timeline:${entry.id}`, lane: entry.lane, status: entry.status, title: entry.title, subtitle: entry.when || entry.status, system: false })),
  ...
  return <main className="campaign-page" aria-label="Campaign dashboard">
    ...
    <section className="campaign-timeline" aria-label="Campaign timeline">
      ...
      <section className="visual-timeline" aria-label="Visual campaign timeline"><div className="visual-timeline-heading"><h3>Visual progression</h3><span>Main campaign <i className="visual-main" /> Quest <i className="visual-quest" /> Backstory <i className="visual-backstory" /></span></div>{visualEvents.length ? <div className="visual-timeline-track">{visualEvents.map((event) => <article className={`visual-event lane-${event.lane} ${event.system ? 'is-system' : ''}`} key={event.id}><i /><strong>{event.title}</strong><span>{event.subtitle}</span>{event.system && <small>World Event</small>}</article>)}</div> : <p>Create a story beat or complete a structured action to see progression here.</p>}</section>
