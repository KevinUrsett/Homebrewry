...
function parsePlotBoard(value: unknown): PlotBoard {
  ...
  const beats = value.beats.map((beat) => {
    ...
    const phaseId = requiredString(beat.phaseId, 'plot board beat phase');
    const storedSpanPhaseIds = beat.spanPhaseIds === undefined ? [] : requiredStringArray(beat.spanPhaseIds, 'plot board beat spanning phases');
    if (storedSpanPhaseIds.some((id) => !phaseIds.has(id))) throw new Error('Campaign data has an invalid plot board beat span.');
    const spanPhaseIds = [...new Set([phaseId, ...storedSpanPhaseIds])];
    return { id: requiredString(beat.id, 'plot board beat ID'), laneId, ...(spanPhaseIds.length > 1 ? { spanPhaseIds } : {}), phaseId, title: requiredString(beat.title, 'plot board beat title'), notes: requiredString(beat.notes, 'plot board beat notes'), status: beat.status as PlotBoard['beats'][number]['status'], entityIds: requiredStringArray(beat.entityIds, 'plot board beat entity IDs'), order: nullableNumber(beat.order, 'plot board beat order') ?? 0, createdAt: requiredString(beat.createdAt, 'plot board beat creation time'), updatedAt: requiredString(beat.updatedAt, 'plot board beat update time') };
  });
