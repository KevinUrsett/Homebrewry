    ...
    {!draft ? <div className="plot-board-empty"><h3>Start a plot board</h3><p>Build the campaign outline manually. Existing timeline entries are not included.</p><button className="primary-button" onClick={createBoard} type="button">Start plot board</button></div> : <>
      ...
      {!phases.length || !lanes.length ? <div className="plot-board-guidance"><strong>{!phases.length ? 'Add your first phase.' : 'Add your first story arc.'}</strong><span>Phases are vertical campaign sections; arcs are horizontal narrative tracks.</span></div> : <>
        ...
        <div className="plot-board-scroll"><div className="plot-board-grid" style={{ '--phase-count': phases.length } as CSSProperties}>
          ...
          {lanes.flatMap((lane, laneIndex) => [<div className={`plot-board-lane tone-${lane.tone}`} key={`${lane.id}:label`}><strong>{lane.title}</strong><span>{lane.tone}</span><div className="plot-board-item-actions plot-board-lane-actions"><button aria-label={`Move ${lane.title} up`} disabled={laneIndex === 0} onClick={() => moveLane(lane.id, -1)} type="button">↑</button><button aria-label={`Insert a story arc after ${lane.title}`} onClick={() => prepareLaneInsert(lane.id)} type="button">+</button><button aria-label={`Move ${lane.title} down`} disabled={laneIndex === lanes.length - 1} onClick={() => moveLane(lane.id, 1)} type="button">↓</button><button aria-label={`Remove ${lane.title}`} disabled={draft.beats.some((beat) => beat.laneId === lane.id)} onClick={() => removeLane(lane.id)} type="button">×</button></div></div>, ...phases.map((phase) => <div className="plot-board-cell" key={`${lane.id}:${phase.id}`}><div className="plot-board-beats">{beats.filter((beat) => beat.laneId === lane.id && beat.phaseId === phase.id).map((beat) => <button className={`plot-board-beat status-${beat.status}`} key={beat.id} onClick={() => openBeat(beat)} type="button"><span>{statusLabel[beat.status]}</span><strong>{beat.title}</strong>{beat.entityIds.length > 0 && <small>{beat.entityIds.map((id) => entityById.get(id)?.name).filter(Boolean).join(', ')}</small>}{beatConnections(beat.id).map((connection) => <em key={connection}>→ {connection}</em>)}</button>)}</div><button aria-label={`Add a beat to ${lane.title} in ${phase.title}`} className="plot-board-add-beat" onClick={() => openNewBeat(lane.id, phase.id)} type="button">+ Beat</button></div>)])}
        </div></div>
      </>}
