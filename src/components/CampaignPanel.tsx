...
export function CampaignPanel({ position, partyLocation, brews, encounters, entities, currentStateByEntityId, worldEvents, timelineEntries, campaignMap, entityReferences, worldbuildingEntries, currentBrewId, onOpenEncounter, onOpenEntity, onOpenBrewSection, onSetCurrentBrew, onOpenWorldbuildingEntry, timelineDraftSeed, onTimelineDraftSeedApplied, onSaveTimelineEntry, onDeleteTimelineEntry, onSaveCampaignMap }: CampaignPanelProps) {
  ...
  const brewById = useMemo(() => new Map(brews.map((brew) => [brew.id, brew])), [brews]);
  const entitiesById = new Map(entities.map((entity) => [entity.id, entity]));
  const encounterById = new Map(encounters.map((encounter) => [encounter.id, encounter]));
  const visualEvents = useMemo(() => [
    ...orderedTimelineEntries.map((entry) => ({ id: `timeline:${entry.id}`, lane: entry.lane, status: entry.status, title: entry.title, subtitle: timelineDateLabel(entry) || entry.status, system: false, brewId: entry.brewId, sectionId: entry.sectionId, worldbuildingId: entry.worldbuildingId, brewAvailable: !entry.brewId || brewById.has(entry.brewId) })),
    ...worldEvents.map((event) => ({ id: `event:${event.id}`, lane: 'main' as const, status: 'past' as const, title: event.type.replaceAll('.', ' '), subtitle: new Date(event.occurredAt).toLocaleString(), system: true, brewId: undefined, sectionId: undefined, worldbuildingId: undefined, brewAvailable: true }))
  ], [brewById, orderedTimelineEntries, worldEvents]);
  ...
  return <main className="campaign-page" aria-label="Campaign dashboard">
    ...
    <section className="campaign-timeline" aria-label="Campaign timeline">
      ...
      <section className="visual-timeline" aria-label="Visual campaign timeline"><div className="visual-timeline-heading"><h3>Campaign tree</h3><span>Main campaign <i className="visual-main" /> Quest <i className="visual-quest" /> Backstory <i className="visual-backstory" /></span></div>{visualEvents.length ? <div className="campaign-tree"><div className="campaign-tree-columns">{visualEvents.map((event) => <div className={`tree-column lane-${event.lane}`} key={event.id}><span className="tree-connector" /><button aria-label={event.brewId || event.worldbuildingId ? `Open ${event.title}` : event.title} className={`tree-node ${event.system ? 'is-system' : ''}`} disabled={!event.brewAvailable || (!event.brewId && !event.worldbuildingId)} onClick={() => event.brewId ? onOpenBrewSection(event.brewId, event.sectionId) : event.worldbuildingId ? onOpenWorldbuildingEntry(event.worldbuildingId) : undefined} type="button"><i /><strong>{event.title}</strong><span>{event.brewAvailable ? event.subtitle : 'Linked brew unavailable'}</span>{event.system && <small>World Event</small>}</button></div>)}</div></div> : <p>Create a story beat or complete a structured action to see progression here.</p>}</section>
      <div className="timeline-list">{timeline.map((entry) => { const encounter = entry.encounterId ? encounterById.get(entry.encounterId) : undefined; const linkedBrew = entry.brewId ? brewById.get(entry.brewId) : undefined; return <article className={`timeline-entry lane-${entry.lane} status-${entry.status}`} key={entry.id}><div className="timeline-entry-meta"><span>{entry.lane === 'main' ? 'Main campaign' : entry.lane === 'quest' ? 'Quest / side story' : 'Character backstory'}</span><b>{entry.status}</b></div><div><h3>{entry.title}</h3>{timelineDateLabel(entry) && <p className="timeline-when">{timelineDateLabel(entry)}</p>}{entry.notes && <p>{entry.notes}</p>}{entry.entityIds.length > 0 && <p className="timeline-links">{entry.entityIds.flatMap((id) => { const entity = entitiesById.get(id); return entity ? [<button key={id} onClick={() => onOpenEntity(entity)} type="button">{entity.name}</button>] : []; })}</p>}{encounter && <button className="timeline-encounter-link" onClick={() => onOpenEncounter(encounter)} type="button">Open {encounter.name}</button>}{linkedBrew && <button className="timeline-encounter-link" onClick={() => onOpenBrewSection(linkedBrew.id, entry.sectionId)} type="button">Open linked brew{entry.sectionId ? ' section' : ''}</button>}{entry.brewId && !linkedBrew && <span className="timeline-missing-link">Linked brew is no longer available.</span>}{entry.worldbuildingId && <button className="timeline-encounter-link" onClick={() => onOpenWorldbuildingEntry(entry.worldbuildingId!)} type="button">Open Worldbuilding entry</button>}</div><button className="quiet-danger" onClick={() => onDeleteTimelineEntry(entry.id)} type="button">Remove</button></article>; })}</div>
